...
public class StringUtil {
	 ...
	 * @see com.nx.commons.lang.Consts#DFT_STRING_VAL
	 */
	public static String getString(Object input) {
		return getString(input, Consts.DFT_STRING_VAL);
	}
	...
	public static String newString(byte[] bytes) {
		return newString(bytes, Consts.DFT_CHARSET);
	}
...
//			throw new NxException("unsupported Encoding {0}", e, charset);
	...
	public static byte[] getBytes(String input) {
		return getBytes(input, Consts.DFT_CHARSET);
	}
...
//			throw new NxException("不支持的字符集:{0}", e, charset);
			logger.error("不支持的字符集 : " + charset, e);
			return "".getBytes();
		}
	}
    ...
    public static <T> String joinList(String seperator, List<T> params) {
    	if (params == null || params.size() == 0) return Consts.DFT_STRING_VAL;
		if (seperator == null) seperator = Consts.DFT_STRING_VAL;
	...
	public static String join(String seperator, Object... params) {
		if (params == null || params.length == 0) return Consts.DFT_STRING_VAL;
		if (seperator == null) seperator = Consts.DFT_STRING_VAL;
		StringBuilder ret = new StringBuilder();
		for (Object param : params) {
			if (ret.length() > 0) ret.append(seperator);
	...
	public static String toString(Object obj) {
		//TODO 
		return obj.toString();
//		if (obj == null) return Consts.DFT_NULL_STRING_VAL;
//		BeanOperation bop = BeanOperationFacade.getPojoParser(obj.getClass());
//		List<String> values = CollectionUtil.newArrayList();
//		for (FieldOperation fop : bop.getFieldOperations()) {
//			values.add("$[" + fop.fieldName() + "=" + fop.getValue(obj) + "]");
//		}
//		return "${" + obj.getClass() + " - " + joinArray(", ", values.toArray(Consts.EMPTY_STRING_ARRAY)) + "}";
	}
}
